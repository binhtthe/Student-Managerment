        ...
        <script>
            ...
            function paging() {
                ...
                if (currentPage - gap > 1) {
                    $("#paging").append(
                            "<a id='1' href=/ThachThatSchool/admin-news-list?query=" + query
                ...
                for (let i = currentPage - gap; i <= currentPage; i++) {
                    if (i >= 1) {
                        $("#paging").append(
                                "<a id='" + i + "' href=/ThachThatSchool/admin-news-list?query=" + query
                ...
                if (currentPage + gap < totalPage) {
                    $("#paging").append(
                            "<a id='" + totalPage + "' href=/ThachThatSchool/admin-news-list?query=" + query
            ...
            function changePerPage() {
                ...
                var query = $("#query").val();
                var totalsearchednews = $("#totalSearchedNews").val();
                window.location = "/ThachThatSchool/admin-news-list?"
                        + "query=" + query + "&totalsearchednews=" + totalsearchednews + "&newsperpage=" + newsperpage;
            }

            function doEdit(no) {
                window.location = "/ThachThatSchool/admin-news-update?no=" + no;
            }
            ...
            function doDelete(no) {
                ...
                if (checked) {
                    $.ajax({
                        url: "/ThachThatSchool/admin-news-delete",
                        type: "POST",
                        data: {
                            no: no
                        },
